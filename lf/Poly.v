...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?  (Add YES or NO to each line.)
      - [d (b a 5)]           NO:无类型参数
      - [d mumble (b a 5)]    YES:类型为 grumble mumble
      - [d bool (b a 5)]      YES:类型为 grumble bool
      - [e bool true]         YES:类型为 grumble bool
      - [e mumble (b c 0)]    YES:类型为 grumble mumble
      - [e bool (b c 0)]      NO:类型不匹配: bool  mumble
      - [c]                   NO:类型错误: c是mumble, 不是grumble    *)               
(* FILL IN HERE *)
End MumbleGrumble.
(** [] *)
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [| h t IHh'].
  - reflexivity.                 
  - simpl. rewrite IHh'. reflexivity. 
Qed.
...
Proof.
  intros X l1 l2. induction l1 as [| h t IHh'].
  - reflexivity.                 
  - simpl. rewrite IHh'. reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. rewrite IHh'. rewrite app_assoc. reflexivity. 
Qed.
...
Proof.
  intros X l. induction l as [| h t IHh'].
  - reflexivity.                 
  - simpl. rewrite rev_app_distr. simpl. rewrite IHh'. reflexivity. 
Qed.
(** [] *)
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  ...
  | [] => ([], [])  
  | (x, y) :: t =>   
      let (lx, ly) := split t in  (* 拆分剩余元素 *)
      (x :: lx, y :: ly)         (* 将当前元素添加到结果 *)
  end.
...
Fixpoint gtb (n m : nat) : bool :=  (*greater than boolean*)
  match n, m with
  | O, _ => false          
  | S _, O => true         
  | S n', S m' => gtb n' m' 
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => andb (even n) (n >? 7)) l.   (* 同时满足偶数、大于7 *)
...
Definition partition {X : Type}
                   ...
                   : list X * list X :=
  (*以“,”为界*)
  (*使用谓词test过滤列表保留满足test X = true的元素*)
  (*使用negb取反，保留所有test x = false 的元素*)
...
Proof.
  intros X Y f l.
  induction l as [| x l' IHx'].
  - reflexivity. 
  - simpl. rewrite map_fenpei. simpl. rewrite IHx'. reflexivity.
Qed.
(** [] *)
...
Fixpoint flat_map {X Y: Type} (f: X -> list Y) (l: list X)
  ...
  match l with
  | [] => []                      
  | h :: t => (f h) ++ flat_map f t  (* 拼接当前结果与递归结果 *)
  end.
...
Fixpoint map_1 (X Y : Type) (f : X->Y) (l : list X) : list Y :=
  match l with
  | []     => @nil Y           (* "@"强制将隐式参数转为显式参数 *)
  | h :: t => @cons Y (f h) (map_1 X Y f t)  
  end.
