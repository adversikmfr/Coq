...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Proof: (* FILL IN HERE *)
*)
Theorem add_comm_informal : forall n m : nat,
  n + m = m + n.
Proof.
  intros n m. induction n as [| n' IHn']. induction m as [| m' IHm']. reflexivity.
  simpl. rewrite <- IHm'. simpl. reflexivity. simpl. rewrite IHn'. rewrite plus_n_Sm. reflexivity. 
Qed.
...
Theorem mult_1_l : forall n:nat,
  1 * n = n.
Proof.
  assert ( plus_n_0 : forall k, k + 0 = k ).
  {
    intros k.
...
Proof.
  (* assert ( mult_plus_distr_l : forall n m p, n * (m + p) = n * m + n * p ).
  {
    induction n as [| n' IHn'].
