...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  ...
  | h :: t => 
      if h =? v 
      then t  (* 找到第一个匹配项 *)
      else h :: remove_one v t  (* 保留当前元素，继续递归 *)
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  ...
  | h :: t => 
      if h =? v 
      then remove_all v t  (* 跳过当前元素 *)
      else h :: remove_all v t  (* 保留当前元素 *)
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil => true   
  | h :: t => 
      if count h s1 <=? count h s2
      then included t s2  (* 当前元素数量足够，检查剩余元素 *)
      else false          (* 当前元素在s2中数量不足 *)
  end.
...
Theorem add_inc_count : forall (a : nat) (b : bag),
    count a (add a b) = 1 + count a b.
Proof.
  intros a b.                 
  simpl.                      
  assert (H: eqb a a = true). { (* 证明 a =? a = true ,然后左侧简化为S (count a b) *)
...
Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l.
Proof.
  induction l as [|h g IHh'].
  - reflexivity.
  - simpl. rewrite rev_app_distr. rewrite IHh'.
    (* 单元素列表反转 *)
    assert (H: forall n, rev [n] = [n]).
...
Theorem bag_count_sum : forall (v : nat) (s1 s2 : bag),
    count v (s1 ++ s2) = count v s1 + count v s2.
Proof.
  intros v s1 s2.
...
Proof.
  intros f H_involution n1 n2 H. 
  rewrite H_involution.
  rewrite <- H.
  rewrite <- H_involution.
  reflexivity.
Qed.
